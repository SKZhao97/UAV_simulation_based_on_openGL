/*
Author: From ECE 6122 sample code
Class: ECE 6122
Last date modified: 11/25/2019
Description: Import .bmp file and process the image
*/
#ifndef Bitmap_h
#define Bitmap_h
...
//                    std::cerr << "Error! The file \"" << fname << "\" does not seem to contain bit mask information\n";
    ...
    BMP(int32_t width, int32_t height, bool has_alpha = true) {
        ...
        if (has_alpha) {
            ...
            file_header.file_size = int(file_header.offset_data + data.size());
        }
        else {
            bmp_info_header.size = sizeof(BMPInfoHeader);
            file_header.offset_data = sizeof(BMPFileHeader) + sizeof(BMPInfoHeader);
            ...
            uint32_t new_stride = make_stride_aligned(4);
            file_header.file_size = int (file_header.offset_data + data.size() + bmp_info_header.height * (new_stride - row_stride));
        }
    }
